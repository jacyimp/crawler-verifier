...
final class CoverageEdgeCasesTest extends TestCase
{
    ...
    {
        $identity = new class implements CrawlerIdentity {
        ...
        $provider = new class implements CrawlerProvider {
            ...
            public function verify(CrawlerIdentity $crawler, string $ip): ?VerificationMethod
            {
                return null;
            }
        };
    ...
    public function cachingResolverToleratesFailuresAndMalformedValues(): void
    {
        $dns = new class implements DnsResolver {
            ...
            {
                return $ip === '' ? null : 'crawler.example';
            }
        ...
        $reverseKey = 'crawler_verifier.dns.reverse.'
            . hash('sha256', bin2hex((string) inet_pton('192.0.2.1')));
    ...
    {
        self::assertNull(
            (new PsrCacheIpRangeSource(new FaultyCache(throwOnGet: true)))
                ->entryFor(Crawler::GPTBot),
        );
    }
    ...
    public function updaterReportsUnsupportedCrawlersAndCacheFailures(): void
    {
        ...
        self::assertArrayHasKey(
            ...
            $unsupported->refreshIfStale(60, Crawler::PinterestBot)->errors,
        );
    ...
    {
        $source = new class implements IpRangeSource {
        ...
        self::assertNull(
            (new BuiltInCrawlerProvider(new BuiltInCrawlerCatalog([]), $source))
                ->verify(Crawler::Googlebot, '192.0.2.1'),
        );
    }
}
